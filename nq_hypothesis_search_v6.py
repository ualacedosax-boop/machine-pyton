...
def score_signals_vs_labels(signals, labels, tolerance_bars=1):
    ...
    precision_soft = matched / (matched + 0.30 * extra) if (matched + extra) > 0 else 0.0
    score = 0.82 * recall + 0.18 * precision_soft
...
def build_signals(df, p):
    ...
    # 0 = candle de confirmação do pivô; 1 = próximo candle após confirmação
    low_conf_idx = pl.shift(p["pivot_right"]).fillna(False)
    high_conf_idx = ph.shift(p["pivot_right"]).fillna(False)
    if p["entry_delay_buy"] == 1:
        low_conf_idx = low_conf_idx.shift(1).fillna(False)
    if p["entry_delay_sell"] == 1:
        high_conf_idx = high_conf_idx.shift(1).fillna(False)

    pivot_low_price = x["low"].shift(p["pivot_right"] + p["entry_delay_buy"])
    pivot_high_price = x["high"].shift(p["pivot_right"] + p["entry_delay_sell"])

    buy = low_conf_idx & (x["body_ratio"] >= p["min_body_buy"])
    sell = high_conf_idx & (x["body_ratio"] >= p["min_body_sell"])
    ...
    buy_rsi = choose_rsi(x, p["rsi_len_buy"]).shift(p["pivot_right"]) <= p["rsi_buy_max"]
    sell_rsi = choose_rsi(x, p["rsi_len_sell"]).shift(p["pivot_right"]) >= p["rsi_sell_min"]
    if p["use_rsi_turn_buy"]:
        buy_rsi = buy_rsi | (choose_rsi(x, p["rsi_len_buy"]) > choose_rsi(x, p["rsi_len_buy"]).shift(1))
    if p["use_rsi_turn_sell"]:
        sell_rsi = sell_rsi | (choose_rsi(x, p["rsi_len_sell"]) < choose_rsi(x, p["rsi_len_sell"]).shift(1))
    buy &= buy_rsi
    sell &= sell_rsi
    ...
    if p["use_extreme_buy"]:
        buy &= pivot_low_price <= x["low"].rolling(p["extreme_lb_buy"]).min().shift(p["pivot_right"] + p["entry_delay_buy"])
    if p["use_extreme_sell"]:
        sell &= pivot_high_price >= x["high"].rolling(p["extreme_lb_sell"]).max().shift(p["pivot_right"] + p["entry_delay_sell"])
    ...
    for i in range(len(x)):
        ...
        if bool(sell.iloc[i]) and i - last_sell > p["gap_sell"]:
            if sig.iloc[i] == "BUY":
                # se ambos no mesmo candle, deixa o lado cujo label aparece na lista mais perto do candle
                sig.iloc[i] = "SELL"
                last_sell = i
            else:
                sig.iloc[i] = "SELL"
...
def stage_space():
    return {
        ...
        "pivot_right": [1],
        "entry_delay_buy": [0,1],
        "entry_delay_sell": [0,1],
        "min_body_buy": [0.15,0.20,0.25],
        "min_body_sell": [0.15,0.20,0.25],
        ...
        "min_wick_buy": [0.0,0.10,0.20],
        "min_wick_sell": [0.0,0.10,0.20],
        "rsi_len_buy": [7,10],
        "rsi_len_sell": [7,10],
        "rsi_buy_max": [35,38,40,42],
        "rsi_sell_min": [50,52,55],
        ...
        "ema_dist_buy": [5,9,13],
        "ema_dist_sell": [5,9,13],
        "min_atr_ema_buy": [0.0,0.10,0.15],
        "min_atr_ema_sell": [0.0,0.10,0.15],
...
def search(df, labels, tolerance_bars=1, max_tests=18000):
    space = stage_space()
...
def main():
    ...
    ap.add_argument("--outdir", default="saida_hypothesis_v6")
    ap.add_argument("--tolerance-bars", type=int, default=1)
    ap.add_argument("--max-tests", type=int, default=18000)
    args = ap.parse_args()
    ...
    report = []
    report.append("=== HYPOTHESIS SEARCH V6 ===")
    report.append("Busca focada nas entradas do video")
