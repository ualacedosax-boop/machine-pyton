...
ARQ_FEATURES_2026 = PASTA_SAIDA / "01_2026_features_config_antiga.csv.gz"
ARQ_SCORE_2026 = PASTA_SAIDA / "02_2026_score_config_antiga.csv.gz"
ARQ_TRADES_2026 = PASTA_SAIDA / "03_2026_trades_config_antiga.csv.gz"
ARQ_RESUMO_2026 = PASTA_SAIDA / "04_2026_resumo_config_antiga.csv"
ARQ_ANALISE_2026 = PASTA_SAIDA / "05_2026_analise_config_antiga.csv"


# ============================================================
# CONFIGURAÇÃO ANTIGA FORA DA AMOSTRA
# ============================================================
...
def calcular_features_base(df):
    ...
    for n in [2, 3, 4, 5, 8, 10]:
        df[f"seq_alta_{n}"] = (df["candle_alta"].rolling(n).sum() == n).astype(int)
        df[f"seq_baixa_{n}"] = (df["candle_baixa"].rolling(n).sum() == n).astype(int)
    ...
    media_barcount_horario = df.groupby(hora_minuto)["barCount"].transform("mean")

    df["volume_relativo_horario"] = safe_div(df["volume"], media_volume_horario)
    df["barcount_relativo_horario"] = safe_div(df["barCount"], media_barcount_horario)

    # EMAs
    for n in [5, 8, 9, 13, 17, 20, 21, 34, 50, 72, 100, 144, 200]:
    ...
    df["dist_ema17_34_pct"] = safe_div(df["dist_ema17_34"], df["close"]) * 100.0
    ...
    for n in [8, 14, 21]:
        ...
        raw = safe_div(r - r_min, r_max - r_min) * 100.0
        k = pd.Series(raw, index=df.index).rolling(3).mean()
        d = k.rolling(3).mean()

        df[f"stochrsi_{n}_raw"] = raw
    ...
    for n in [20, 34]:
        ...
        df[f"kc_mid_{n}"] = mid
        df[f"kc_upper_{n}"] = upper
        df[f"kc_lower_{n}"] = lower
    ...
    df["cum_tpv_dia"] = df.groupby("Data")["tpv"].cumsum()
    df["cum_vol_dia"] = df.groupby("Data")["volume"].cumsum()
...
def main():
    print("=====================================================")
    print("VALIDAÇÃO V4 2026 - CONFIGURAÇÃO ANTIGA")
    print("=====================================================")
    ...
    trades = aplicar_config_antiga(score_resultado)
    ...
    print("\n=====================================================")
    print("RESULTADO FORA DA AMOSTRA 2026 - CONFIGURAÇÃO ANTIGA")
    print("=====================================================")
