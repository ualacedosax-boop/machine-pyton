...
ARQUIVO_CANDLES_2024 = (
    BASE_DIR
    / "dados_mnq_2024_ibkr"
    / "MNQ_2024_2MIN_IBKR_CONTINUO.csv"
)

ARQUIVO_CANDLES_2025 = (
    BASE_DIR
    / "dados_mnq_2025_ibkr"
    / "MNQ_2025_2MIN_IBKR_CONTINUO.csv"
)
...
PASTA_SAIDA = BASE_DIR / "saida_v5_filtro_campeao_2024_2025_2026"
PASTA_SAIDA.mkdir(parents=True, exist_ok=True)

ARQ_DATASET_2024 = PASTA_SAIDA / "01_dataset_v5_campeao_2024.csv.gz"
ARQ_DATASET_2025 = PASTA_SAIDA / "02_dataset_v5_campeao_2025.csv.gz"
ARQ_DATASET_2026 = PASTA_SAIDA / "03_dataset_v5_campeao_2026.csv.gz"

ARQ_DATASET_TREINO_2024_2025 = PASTA_SAIDA / "04_dataset_v5_TREINO_2024_2025.csv.gz"
ARQ_DATASET_TESTE_2026 = PASTA_SAIDA / "05_dataset_v5_TESTE_2026.csv.gz"
ARQ_DATASET_TUDO = PASTA_SAIDA / "06_dataset_v5_TUDO_2024_2025_2026.csv.gz"

ARQ_RESUMO = PASTA_SAIDA / "07_resumo_dataset_v5_2024_2025_2026.csv"
ARQ_FEATURES_SUGERIDAS = PASTA_SAIDA / "08_features_sugeridas_v5.txt"
...
def adicionar_features_v5(df):
    ...
    out["Dia"] = out["DataHora_SP"].dt.day
    out["DiaSemana"] = out["DataHora_SP"].dt.dayofweek
...
def calcular_drawdown_trades(df):
    if df.empty:
        return 0.0
...
def gerar_dataset_ano(arquivo_candles, ano_nome):
    ...
    if not Path(arquivo_candles).exists():
        raise FileNotFoundError(f"Arquivo não encontrado: {arquivo_candles}")
...
def main():
    print("=====================================================")
    print("01B - GERAR DATASET V5 COM 2024 + 2025 + 2026")
    print("=====================================================")
    ...
    arquivo_2024 = ARQUIVO_CANDLES_2024
    arquivo_2025 = localizar_candles_2025()
    arquivo_2026 = ARQUIVO_CANDLES_2026

    dataset_2024 = gerar_dataset_ano(arquivo_2024, "2024")
    dataset_2025 = gerar_dataset_ano(arquivo_2025, "2025")
    dataset_2026 = gerar_dataset_ano(arquivo_2026, "2026")

    if not dataset_2024.empty:
        dataset_2024.to_csv(ARQ_DATASET_2024, index=False, compression="gzip")
        print("\nDataset 2024 salvo:")
        print(ARQ_DATASET_2024)
    ...
    treino_2024_2025 = pd.concat(
        [d for d in [dataset_2024, dataset_2025] if not d.empty],
        ignore_index=True
    )

    teste_2026 = dataset_2026.copy()

    todos = pd.concat(
        [d for d in [dataset_2024, dataset_2025, dataset_2026] if not d.empty],
        ignore_index=True
    )

    treino_2024_2025 = treino_2024_2025.sort_values("DataHora_SP").reset_index(drop=True)
    teste_2026 = teste_2026.sort_values("DataHora_SP").reset_index(drop=True)
    todos = todos.sort_values("DataHora_SP").reset_index(drop=True)

    treino_2024_2025.to_csv(ARQ_DATASET_TREINO_2024_2025, index=False, compression="gzip")
    teste_2026.to_csv(ARQ_DATASET_TESTE_2026, index=False, compression="gzip")
    todos.to_csv(ARQ_DATASET_TUDO, index=False, compression="gzip")

    resumo = pd.DataFrame([
        resumir(dataset_2024, "2024"),
        resumir(dataset_2025, "2025"),
        resumir(dataset_2026, "2026_TESTE_FINAL"),
        resumir(treino_2024_2025, "TREINO_2024_2025"),
        resumir(todos, "TUDO_2024_2025_2026"),
    ])

    resumo.to_csv(ARQ_RESUMO, index=False)

    print("\n=====================================================")
    print("RESUMO DATASET V5 - 2024 + 2025 + 2026")
    print("=====================================================")
    print(resumo.to_string(index=False))

    salvar_features_sugeridas(todos)

    print("\nArquivos gerados:")
    print(ARQ_DATASET_2024)
    print(ARQ_DATASET_2025)
    print(ARQ_DATASET_2026)
    print(ARQ_DATASET_TREINO_2024_2025)
    print(ARQ_DATASET_TESTE_2026)
    print(ARQ_DATASET_TUDO)
    print(ARQ_RESUMO)
    print(ARQ_FEATURES_SUGERIDAS)

    print("\nPróxima etapa:")
    print("Treinar V5 usando TREINO_2024_2025 e testar separado em 2026.")
