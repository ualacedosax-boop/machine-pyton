...
def limpar_setup_familia(s: pd.Series) -> pd.Series:
    s = s.copy()
    s = s.fillna("")
    s = s.astype(str).str.strip().str.lower()
    s = s.replace({"nan": "", "none": "", "null": "", "<na>": ""})
    return s
...
def main():
    ...
    df["setup_familia"] = limpar_setup_familia(df["setup_familia"])

    df = df.dropna(subset=["datetime"])
    validos = df[df["setup_familia"] != ""].copy()

    if validos.empty:
        resumo_familia = pd.DataFrame(columns=["setup_familia", "BUY", "SELL"])
    else:
        resumo_familia = (
    ...
    if "qualidade_1a5" in validos.columns and not validos.empty:
    ...
    texto.append("")
    if validos.empty:
        texto.append("Nenhuma entrada foi classificada ainda. Preencha a coluna setup_familia.")
    else:
