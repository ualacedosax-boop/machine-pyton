...
def _try_read_csv(path: Path, header, **kwargs):
...
def read_any(path: Path) -> pd.DataFrame:
    ...
    attempts = [
        ...
        {"encoding": "utf-8", "sep": None, "engine": "python", "header": 0},
        {"encoding": "utf-8-sig", "sep": None, "engine": "python", "header": 0},
        {"encoding": "utf-16", "sep": None, "engine": "python", "header": 0},
        {"encoding": "latin1", "sep": None, "engine": "python", "header": 0},
        {"encoding": "cp1252", "sep": None, "engine": "python", "header": 0},
    ]
    ...
    for kwargs in attempts:
        try:
            ...
            if len(df.columns) == 1 and "sep" not in kwargs:
                continue
            return df
...
def read_any_headerless(path: Path) -> pd.DataFrame:
    if path.suffix.lower() in [".xlsx", ".xls"]:
        raise ValueError("Leitura sem cabeçalho não suportada para Excel.")
...
def _looks_like_datetime_series(s: pd.Series) -> bool:
    ...
    if not sample:
        return False
    return any((":" in x or "T" in x or "-" in x or "/" in x) for x in sample)
...
def _is_headerless_ohlc(df: pd.DataFrame) -> bool:
    ...
    if len(cols) < 5:
        return False
    first_col_ok = _looks_like_datetime_series(pd.Series(cols[:1])) or _looks_like_datetime_series(df.iloc[:, 0])
    numeric_like_headers = sum(_is_number_like(c) for c in cols[1:min(len(cols), 7)])
    return first_col_ok and numeric_like_headers >= 4
...
def load_ohlc(path: Path) -> pd.DataFrame:
    ...
    # Caso comum: arquivo sem cabeçalho; o pandas usou a 1ª linha como nome de coluna
    if _is_headerless_ohlc(df):
        raw = read_any_headerless(path)
        if raw.shape[1] >= 5:
            new_cols = ["datetime", "open", "high", "low", "close"]
            if raw.shape[1] >= 6:
                new_cols.append("volume")
            if raw.shape[1] >= 7:
                new_cols.append("extra")
            raw.columns = new_cols[: raw.shape[1]]
            df = raw.copy()
    ...
    for c in df.columns:
        if c in ["date", "data"]:
            rename_map[c] = "date"
        elif c in ["time", "hora", "timestamp", "datahora", "data_hora", "date_time", "datetime"]:
            ...
            rename_map[c] = "close"
        elif c in ["volume", "vol", "tick_volume", "contracts"]:
            rename_map[c] = "volume"
    df = df.rename(columns=rename_map)

    # fallback: primeira coluna parece datetime
    ...
    # fallback: 5 ou 6 colunas sem nomes padrão, mas formato OHLC
    ...
    if "datetime" in df.columns:
        ...
        raise ValueError(f"Arquivo {path.name}: nao encontrei datetime nem date+time. Colunas encontradas: {list(df.columns)}")

    for c in ["open", "high", "low", "close"]:
        if c not in df.columns:
            raise ValueError(f"Arquivo {path.name}: falta coluna {c}. Colunas encontradas: {list(df.columns)}")
...
def run_ml(df: pd.DataFrame, features: list[str]):
    ...
    clf = RandomForestClassifier(n_estimators=300, max_depth=4, min_samples_leaf=3, random_state=42, class_weight="balanced_subsample")
...
DEFAULT_FEATURES = [
    "b_ret_3", "b_ret_5", "b_rsi_7", "b_rsi_9", "b_rsi_14", "b_atr_14", "b_atr_pct", "b_cci_20",
    "b_dist_top_5", "b_dist_bottom_5", "b_pos_range_5", "b_dist_top_10", "b_dist_bottom_10", "b_pos_range_10",
    "b_dist_sma_8", "b_dist_sma_9", "b_slope_ema_5_5", "b_slope_ema_21_5", "b_bull_reject", "b_bear_reject",
    "b_pivot_low", "b_pivot_high", "b_run_up_3", "b_run_dn_3", "b_run_up_5", "b_run_dn_5",
    "m5_rsi_7", "m5_rsi_9", "m5_dist_top_5", "m5_dist_bottom_5", "m5_pos_range_5", "m5_bull_reject",
    "m5_bear_reject", "m5_pivot_low", "m5_pivot_high", "m5_close_pos_bar", "m5_run_up_3", "m5_run_dn_3",
    "h1_rsi_14", "h1_dist_sma_21", "h1_dist_ema_21", "h1_dist_sma_50", "h1_dist_ema_50", "h1_pos_range_10",
    "h1_pos_range_20", "h1_slope_ema_21_5", "h1_slope_ema_5_5", "h1_bull_reject", "h1_bear_reject",
    "b_sess_asia", "b_sess_europa", "b_sess_ny_open", "b_sess_ny_mid", "b_sess_ny_close", "b_hour_sin", "b_hour_cos",
]

OPTIONAL_TF30 = ["m30_rsi_14", "m30_pos_range_10", "m30_dist_sma_21", "m30_slope_ema_21_5", "m30_bull_reject", "m30_bear_reject"]


def write_summary(path: Path, df: pd.DataFrame, ranking: pd.DataFrame, metrics: dict, not_matched: int) -> None:
    lines = [
        "RESUMO DA ANALISE V4 MULTI-TIMEFRAME",
        "",
    ...
    lines += [
        ...
        f"- roc_auc: {'nan' if pd.isna(metrics.get('roc_auc', float('nan'))) else f'{metrics.get('roc_auc'):.4f}'}",
...
def main():
    ...
    ap.add_argument("--tf30", required=False, default=None)
    ...
    pd.DataFrame(metrics["confusion_matrix"], index=["real_sell", "real_buy"], columns=["pred_sell", "pred_buy"]).to_csv(outdir / "05_matriz_confusao_mtf.csv", encoding="utf-8-sig")
