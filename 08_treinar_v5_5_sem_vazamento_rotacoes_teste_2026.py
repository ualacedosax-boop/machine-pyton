...
PASTA_SAIDA = BASE_DIR / "saida_v5_5_sem_vazamento_rotacoes_teste_2026"
PASTA_SAIDA.mkdir(parents=True, exist_ok=True)

ARQ_FEATURES_REMOVIDAS = PASTA_SAIDA / "00_features_removidas_vazamento_v5_5.csv"
ARQ_FEATURES_VALIDAS = PASTA_SAIDA / "00_features_validas_sem_vazamento_v5_5.csv"
ARQ_LEADERBOARD_ROTACOES = PASTA_SAIDA / "01_leaderboard_rotacoes_v5_5.csv"
ARQ_RESUMO_CONFIGS = PASTA_SAIDA / "02_resumo_configs_v5_5.csv"
ARQ_RESULTADO_2026 = PASTA_SAIDA / "03_resultado_teste_2026_v5_5.csv"
ARQ_MENSAL_ROTACOES = PASTA_SAIDA / "04_mensal_rotacoes_v5_5.csv"
ARQ_MENSAL_2026 = PASTA_SAIDA / "05_mensal_teste_2026_v5_5.csv"
ARQ_PRED_2026 = PASTA_SAIDA / "06_predicoes_teste_2026_v5_5.csv.gz"
ARQ_CONFIG = PASTA_SAIDA / "07_config_v5_5.json"
ARQ_MODELO_FINAL = PASTA_SAIDA / "modelo_final_v5_5.joblib"
ARQ_FEATURES_FINAL = PASTA_SAIDA / "features_final_v5_5.joblib"
ARQ_IMPORTANCIA = PASTA_SAIDA / "08_importancia_features_v5_5.csv"
...
BENCHMARKS = {
    ...
    "V5_4_2026": {
        ...
        "winrate": 80.419580,
        "lucro_pontos": 2531.5,
        "profit_factor": 1.772741,
        "drawdown_trades": -447.0,
    },
}
...
def feature_eh_vazamento(nome):
    ...
    for termo in COLUNAS_PROIBIDAS_RESULTADO:
...
def carregar_features_base(df_total):
    ...
    for col in features_originais:
        ...
        if col not in df_total.columns:
            motivo = "nao_existe_no_dataset"
        elif df_total[col].dtype.kind not in "biufc":
            motivo = "nao_numerica"
        elif feature_eh_vazamento(col):
            motivo = "vazamento_temporal_contrato_resultado"
    ...
    pd.DataFrame(removidas).to_csv(ARQ_FEATURES_REMOVIDAS, index=False)
    pd.DataFrame({"feature": validas}).to_csv(ARQ_FEATURES_VALIDAS, index=False)

    if not validas:
        raise RuntimeError("Nenhuma feature válida após remover vazamentos.")
...
def treinar_global(df_train, df_pred, features, modelo_nome):
    ...
    pred = df_pred.copy()
    pred["prob_v5_5"] = obter_prob(modelo, X_pred)
    pred["modelo_v5_5"] = modelo_nome
    pred["modo_v5_5"] = "GLOBAL"
...
def treinar_direcao(df_train, df_pred, features, modelo_nome):
    pred = df_pred.copy()
    pred["prob_v5_5"] = np.nan
    pred["modelo_v5_5"] = modelo_nome
    pred["modo_v5_5"] = "DIRECAO_SEPARADA"
    ...
    for direcao in ["BUY", "SELL"]:
        ...
        pred.loc[idx, "prob_v5_5"] = obter_prob(modelo, X_pred)
        modelos[direcao] = modelo

    pred["prob_v5_5"] = pred["prob_v5_5"].fillna(0.0)
...
def avaliar_threshold(pred, threshold, nome):
    aceitos = pred[pred["prob_v5_5"] >= threshold].copy()

    r = resumo(aceitos, nome)

    r["threshold"] = threshold
    r["trades_cortados"] = int(len(pred) - len(aceitos))
    r["wins_cortados"] = int(((pred[TARGET_COL] == 1) & (pred["prob_v5_5"] < threshold)).sum())
    r["losses_cortados"] = int(((pred[TARGET_COL] == 0) & (pred["prob_v5_5"] < threshold)).sum())
...
def main():
    print("=====================================================")
    print("V5.5 - SEM VAZAMENTO | ROTAÇÕES 2024/2025 | TESTE 2026")
    print("=====================================================")

    df_base, df_2026, df_total = carregar_dados()
    features_base, removidas = carregar_features_base(df_total)

    print("\nDataset:")
    print("Base 2024+2025:", len(df_base))
    print("Teste 2026    :", len(df_2026))
    print("Features válidas sem vazamento:", len(features_base))
    print("Features removidas:", len(removidas))

    print("\nExemplos de features removidas:")
    if removidas:
        print(pd.DataFrame(removidas).head(40).to_string(index=False))
    else:
        print("Nenhuma removida.")

    imp = selecionar_features_por_importancia(df_base, features_base)
    imp.to_csv(ARQ_IMPORTANCIA, index=False)

    print("\nTop 30 features V5.5 sem vazamento:")
                ...
                for nome_rotacao, meses_validacao in ROTACOES.items():
                    ...
                    try:
                        ...
                        for thr in THRESHOLDS:
                            ...
                            aceitos = pred_valid[pred_valid["prob_v5_5"] >= thr].copy()
    ...
    print("\n=====================================================")
    print("RESUMO CONFIGS V5.5 - TOP 40")
    print("=====================================================")
    ...
    print("\n=====================================================")
    print("MELHOR CONFIG V5.5 ESCOLHIDA NAS ROTAÇÕES")
    print("=====================================================")
    ...
    pred_2026["aceito_v5_5"] = pred_2026["prob_v5_5"] >= threshold

    pred_2026.to_csv(ARQ_PRED_2026, index=False, compression="gzip")

    aceitos_2026 = pred_2026[pred_2026["aceito_v5_5"]].copy()
    resultado_2026 = resumo(aceitos_2026, "TESTE_FINAL_2026_V5_5")
    ...
    resultado_2026.update({
        ...
        "supera_v5_3_lucro": bool(resultado_2026["lucro_pontos"] > BENCHMARKS["V5_3_2026"]["lucro_pontos"]),
        "supera_v5_3_pf": bool(resultado_2026["profit_factor"] > BENCHMARKS["V5_3_2026"]["profit_factor"]),
        "melhora_v5_3_dd": bool(resultado_2026["drawdown_trades"] >= BENCHMARKS["V5_3_2026"]["drawdown_trades"]),
        "supera_v5_4_lucro": bool(resultado_2026["lucro_pontos"] > BENCHMARKS["V5_4_2026"]["lucro_pontos"]),
        "supera_v5_4_pf": bool(resultado_2026["profit_factor"] > BENCHMARKS["V5_4_2026"]["profit_factor"]),
        "melhora_v5_4_dd": bool(resultado_2026["drawdown_trades"] >= BENCHMARKS["V5_4_2026"]["drawdown_trades"]),
    })

    resultado_2026_df = pd.DataFrame([resultado_2026])
    resultado_2026_df.to_csv(ARQ_RESULTADO_2026, index=False)

    mensal_2026 = mensal(aceitos_2026, "TESTE_FINAL_2026_V5_5")
    mensal_2026.to_csv(ARQ_MENSAL_2026, index=False)

    print("\n=====================================================")
    print("RESULTADO TESTE FINAL 2026 - V5.5")
    print("=====================================================")
    print(pd.Series(resultado_2026).to_string())

    print("\n=====================================================")
    print("MENSAL TESTE FINAL 2026 - V5.5")
    print("=====================================================")
    ...
    config = {
        "nome": "V5_5_SEM_VAZAMENTO_ROTACOES_TESTE_2026",
        "rotacoes": ROTACOES,
        "features_removidas_total": len(removidas),
        "features_validas_total": len(features_base),
        "melhor_config_rotacoes": melhor,
        "resultado_teste_2026": resultado_2026,
        "benchmarks": BENCHMARKS,
        "features": features_melhor,
        "arquivos": {
            "features_removidas": str(ARQ_FEATURES_REMOVIDAS),
            "features_validas": str(ARQ_FEATURES_VALIDAS),
    ...
    print("=====================================================")
    print(ARQ_FEATURES_REMOVIDAS)
    print(ARQ_FEATURES_VALIDAS)
