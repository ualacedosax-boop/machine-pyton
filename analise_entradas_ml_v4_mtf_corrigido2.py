...
"""
V4 multi-timeframe (versão corrigida 2)
- leitura robusta CSV/XLSX
- fallback de encoding
- reconhecimento mais flexível da coluna de datetime
- base 15m + contexto 5m + 1h + 30m opcional
"""
...
def load_ohlc(path: Path) -> pd.DataFrame:
    ...
    # Se não encontrou datetime, tenta assumir a primeira coluna como datetime
    if "datetime" not in df.columns and len(df.columns) >= 5:
        primeira = df.columns[0]
        amostra = df[primeira].astype(str).head(5).tolist()
        if any((":" in x or "T" in x or "-" in x or "/" in x) for x in amostra):
            df = df.rename(columns={primeira: "datetime"})
