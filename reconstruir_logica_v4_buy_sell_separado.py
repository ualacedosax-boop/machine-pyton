...
# SELL: hipótese já mais forte
SELL_CLOSE_POS_LIST = [0.50, 0.45, 0.40, 0.35]
SELL_K_LIST = [None, 65, 70, 75]
SELL_USAR_KD_LIST = [0, 1]
SELL_USAR_CLOSE_EMA17_LIST = [0, 1]
SELL_USAR_DIST_EMA17_LIST = [0, 1]
SELL_DIST_EMA17_LIST = [0.0, 0.0005, 0.0010]
SELL_USAR_BBPOS_LIST = [0, 1]
SELL_BBPOS_LIST = [0.60, 0.65, 0.70]

# BUY: hipóteses mais abertas
BUY_CLOSE_POS_LIST = [0.50, 0.55, 0.60, 0.65]
BUY_K_LIST = [None, 25, 30, 35, 40]
BUY_USAR_KD_LIST = [0, 1]
BUY_USAR_CLOSE_EMA17_LIST = [0, 1]
BUY_USAR_CLOSE_EMA34_LIST = [0, 1]
BUY_USAR_DIST_EMA17_LIST = [0, 1]
BUY_DIST_EMA17_LIST = [0.0, 0.0005, 0.0010]
BUY_USAR_MACD_LIST = [0, 1]
BUY_USAR_BBPOS_LIST = [0, 1]
BUY_BBPOS_LIST = [0.30, 0.35, 0.40]
BUY_USAR_KCLOWER_LIST = [0, 1]
...
for close_pos_sell, k_sell, usar_kd_sell, usar_close_ema17, usar_dist_ema17, dist_lim, usar_bb, bb_lim in product(
    SELL_CLOSE_POS_LIST,
    SELL_K_LIST,
    SELL_USAR_KD_LIST,
    SELL_USAR_CLOSE_EMA17_LIST,
    SELL_USAR_DIST_EMA17_LIST,
    SELL_DIST_EMA17_LIST,
    SELL_USAR_BBPOS_LIST,
    SELL_BBPOS_LIST
):
    cond = universo["toque_kc_upper"] == 1
    cond = cond & (universo["close_pos"] <= close_pos_sell)
...
ranking_sell = pd.DataFrame(resultados_sell).sort_values(
    ["f1", "precision", "recall", "tp"],
    ascending=False
).reset_index(drop=True)

ranking_sell.to_csv("ranking_sell_v4.csv", index=False)
...
for close_pos_buy, k_buy, usar_kd_buy, usar_close_ema17, usar_close_ema34, usar_dist_ema17, dist_lim, usar_macd, usar_bb, bb_lim, usar_kc_lower in product(
    BUY_CLOSE_POS_LIST,
    BUY_K_LIST,
    BUY_USAR_KD_LIST,
    BUY_USAR_CLOSE_EMA17_LIST,
    BUY_USAR_CLOSE_EMA34_LIST,
    BUY_USAR_DIST_EMA17_LIST,
    BUY_DIST_EMA17_LIST,
    BUY_USAR_MACD_LIST,
    BUY_USAR_BBPOS_LIST,
    BUY_BBPOS_LIST,
    BUY_USAR_KCLOWER_LIST
):
    cond = pd.Series(True, index=universo.index)

    # BUY pode ou não exigir toque no Keltner inferior
    if usar_kc_lower == 1:
        cond = cond & (universo["toque_kc_lower"] == 1)

    cond = cond & (universo["close_pos"] >= close_pos_buy)
...
ranking_buy = pd.DataFrame(resultados_buy).sort_values(
    ["f1", "precision", "recall", "tp"],
    ascending=False
).reset_index(drop=True)

ranking_buy.to_csv("ranking_buy_v4.csv", index=False)

# =========================================================
# SNAPSHOT RESUMO
# =========================================================
snapshots_resumo = snapshots[[
    "datetime", "tipo", "close", "ema17", "ema34",
...
snapshots_resumo.to_csv("snapshot_v4_buy_sell.csv", index=False)

print("\nTop 20 SELL V4:")
print(ranking_sell.head(20))

print("\nTop 20 BUY V4:")
print(ranking_buy.head(20))

print("\nResumo médio das entradas:")
print(
...
print("\nArquivos salvos:")
print("- ranking_sell_v4.csv")
print("- ranking_buy_v4.csv")
print("- snapshot_v4_buy_sell.csv")
