...
PASTA_SAIDA = os.path.join(
    BASE_DIR,
    "saida_ml_entradas_video_v3_otimizacao_takestop_foco_505_117_120"
)
...
# Para mesa proprietÃ¡ria:
# True = depois de 1 loss no dia, para de operar naquele dia.
# False = continua atÃ© bater o limite de entradas do dia.
...
def remover_temporarios():
    ...
    for nome in os.listdir(PASTA_SAIDA):
        if nome.endswith(".tmp"):
            ...
            try:
                os.remove(caminho)
                print("Arquivo temporÃ¡rio removido:", caminho)
            except Exception as e:
                print("NÃ£o conseguiu remover temporÃ¡rio:", caminho, e)
...
def selecionar_trades_por_dia(
    ...
    for data, grupo in candidatos.groupby("Data"):
        ...
        for _, row in grupo.iterrows():
            ...
            # NÃ£o deixa abrir nova operaÃ§Ã£o antes da anterior fechar.
...
if not os.path.exists(ARQUIVO_SCORE):
    raise FileNotFoundError(f"Arquivo nÃ£o encontrado: {ARQUIVO_SCORE}")
...
print("Linhas:", len(df))
print("InÃ­cio:", df["DataHora_SP"].min())
...
if os.path.exists(ARQUIVO_CHECKPOINT):
    ...
    print("ConfiguraÃ§Ãµes jÃ¡ testadas:", len(configs_testadas))
...
print("Total de configuraÃ§Ãµes:", len(configs))
print("JÃ¡ testadas:", len(configs_testadas))
