...
def bollinger(df, length=20, mult=2.0):
    ...
    return basis, upper, lower, width, pos

def linear_slope(series, period=5):
    vals = series.values
...
def add_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    x["cci20"] = cci(x, 20)
    x["roc3"] = roc(x["close"], 3)
    x["roc5"] = roc(x["close"], 5)
    x["macd_hist"] = macd_hist(x["close"])
    x["williams_r"] = williams_r(x, 14)

    bb_basis, bb_upper, bb_lower, bb_width, bb_pos = bollinger(x, 20, 2.0)
    x["bb_width"] = bb_width
    x["bb_pos"] = bb_pos
    ...
    x["ema9_slope5"] = linear_slope(x["ema9"], 5)
    x["ema13_slope5"] = linear_slope(x["ema13"], 5)
    ...
    x["break_high_3"] = (x["close"] > x["high"].shift(1).rolling(3).max()).astype(int)
    x["break_low_3"] = (x["close"] < x["low"].shift(1).rolling(3).min()).astype(int)
    x["range_pos_5"] = (x["close"] - x["low"].rolling(5).min()) / (x["high"].rolling(5).max() - x["low"].rolling(5).min()).replace(0, np.nan)
...
FEATURES = [
    "rsi2","rsi5","rsi7","rsi10","rsi14","k","d",
    "adx14","plus_di","minus_di","cci20","roc3","roc5","macd_hist","williams_r",
    "bb_width","bb_pos",
    "body_ratio","upper_wick_ratio","lower_wick_ratio",
    "dist_ema5_atr","dist_ema9_atr","dist_ema13_atr",
    "green","red","close_up","close_down",
    "ema5_gt_13","ema9_gt_21","ema13_gt_34",
    "ema9_slope5","ema13_slope5",
    "ret1","ret2","ret3",
    "break_high_3","break_low_3","range_pos_5",
    "hour","minute"
]

FEATURE_GROUPS = {
    "core": ["rsi7","body_ratio","upper_wick_ratio","lower_wick_ratio","dist_ema9_atr"],
    "rsi_pack": ["rsi2","rsi5","rsi7","rsi10","rsi14"],
    "stoch_pack": ["k","d"],
    "ema_pack": ["dist_ema5_atr","dist_ema9_atr","dist_ema13_atr","ema5_gt_13","ema9_gt_21","ema13_gt_34","ema9_slope5","ema13_slope5"],
    "candle_pack": ["body_ratio","upper_wick_ratio","lower_wick_ratio","green","red","close_up","close_down"],
    "trend_pack": ["adx14","plus_di","minus_di","ema5_gt_13","ema9_gt_21","ema13_gt_34"],
    "momentum_pack": ["cci20","roc3","roc5","macd_hist","williams_r"],
    "vol_pack": ["bb_width","bb_pos","dist_ema5_atr","dist_ema9_atr","dist_ema13_atr"],
    "break_pack": ["break_high_3","break_low_3","range_pos_5"],
...
def build_dataset(df: pd.DataFrame, labels: pd.DataFrame, neg_mult: int = 4) -> pd.DataFrame:
    ...
    return pd.concat([pos, neg], axis=0).sort_values("time").reset_index(drop=True)
...
def make_feature_subsets() -> List[List[str]]:
    ...
    subsets.append(sorted(set(FEATURE_GROUPS["core"] + FEATURE_GROUPS["trend_pack"] + FEATURE_GROUPS["momentum_pack"])))
    subsets.append(sorted(set(FEATURE_GROUPS["core"] + FEATURE_GROUPS["vol_pack"] + FEATURE_GROUPS["break_pack"])))
    subsets.append(sorted(set(FEATURE_GROUPS["core"] + FEATURE_GROUPS["trend_pack"] + FEATURE_GROUPS["momentum_pack"] + FEATURE_GROUPS["vol_pack"] + FEATURE_GROUPS["break_pack"])))
...
def eval_one(ds: pd.DataFrame, feat_cols: List[str], model_name: str, threshold: float, seed: int):
    ...
    for tr, te in tscv.split(X):
        ...
        if len(unique_classes) < 2:
            const_prob = 1.0 if unique_classes[0] == 1 else 0.0
            oof[te] = const_prob
            continue
        ...
        if hasattr(model, "predict_proba"):
            ...
            if proba.shape[1] == 1:
                ...
                p = np.ones(len(Xte)) if only_class == 1 else np.zeros(len(Xte))
            else:
                class_to_col = {c: i for i, c in enumerate(model.classes_)}
                p = proba[:, class_to_col[1]]
        else:
...
def main():
    ...
    ap.add_argument("--outdir", default="saida_iterative_ml_v2")
    ...
    for i, (feat_cols, model_name, threshold, seed) in enumerate(candidates, start=1):
        ...
        print(
            f"[{i}/{args.max_rounds}] model={model_name} thr={threshold:.2f} "
            f"precision={res['precision']:.3f} recall={res['recall']:.3f} "
            f"f1={res['f1']:.3f} score={res['score']:.3f}"
        )
    ...
    report = []
    report.append("=== ITERATIVE ML SEARCH V2 ===")
    ...
    if best is None:
        ...
        if summary["hit_target"]:
            ...
            report.append("V2 adicionou ADX, CCI, ROC, MACD hist, Williams %R, Bollinger, slopes de EMA e breakouts curtos.")
