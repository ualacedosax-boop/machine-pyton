...
PASTA_SAIDA = BASE_DIR / "saida_v7_v51_filtro_v55_gestao_dia"
PASTA_SAIDA.mkdir(parents=True, exist_ok=True)

ARQ_LEADERBOARD = PASTA_SAIDA / "01_leaderboard_rotacoes_v7.csv"
ARQ_RESUMO_CONFIGS = PASTA_SAIDA / "02_resumo_configs_v7.csv"
ARQ_RESULTADO_2026 = PASTA_SAIDA / "03_resultado_teste_2026_v7.csv"
ARQ_MENSAL_2026 = PASTA_SAIDA / "04_mensal_teste_2026_v7.csv"
ARQ_DIAS_2026 = PASTA_SAIDA / "05_dias_teste_2026_v7.csv"
ARQ_PRED_2026 = PASTA_SAIDA / "06_predicoes_teste_2026_v7.csv.gz"
ARQ_CONFIG = PASTA_SAIDA / "07_config_v7.json"
ARQ_MODELOS_FINAL = PASTA_SAIDA / "modelos_final_v7.joblib"
ARQ_FEATURES_FINAL = PASTA_SAIDA / "features_final_v7.joblib"
ARQ_IMPORTANCIA_V55 = PASTA_SAIDA / "08_importancia_features_v55_v7.csv"
...
# V5.1 é motor principal
PROB_V51_MIN_LIST = [round(x, 3) for x in np.arange(0.540, 0.661, 0.005)]

# V5.5 é filtro de risco
PROB_V55_MIN_LIST = [round(x, 3) for x in np.arange(0.400, 0.651, 0.025)]

MAX_TRADES_DIA_LIST = [1, 2, 3]
PARAR_APOS_LOSS_LIST = [False, True]

# Se True, depois de um loss no dia, bloqueia novos trades.
# Se False, usa só max_trades_dia.
...
def aplicar_gestao_dia(pred, prob_v51_min, prob_v55_min, max_trades_dia, parar_apos_loss):
    temp = pred.copy()

    temp["passou_prob_v51"] = temp["prob_v51"] >= prob_v51_min
    temp["passou_prob_v55"] = temp["prob_v55"] >= prob_v55_min
    temp["candidato_v7"] = temp["passou_prob_v51"] & temp["passou_prob_v55"]
    ...
    for _, row in temp.iterrows():
        ...
        entra = bool(row["candidato_v7"])

        if entra:
            if trades_dia[data] >= max_trades_dia:
                entra = False

        if entra and parar_apos_loss and loss_dia[data]:
            entra = False
    ...
    temp["aceito_v7"] = aceito

    return temp


def avaliar_v7(pred, prob_v51_min, prob_v55_min, max_trades_dia, parar_apos_loss, nome):
    ...
    aceitos = temp[temp["aceito_v7"]].copy()
    ...
    r["wins_cortados"] = int(((temp[TARGET_COL] == 1) & (~temp["aceito_v7"])).sum())
    r["losses_cortados"] = int(((temp[TARGET_COL] == 0) & (~temp["aceito_v7"])).sum())
...
def score_config(r):
    ...
    score += r["dd_pior_validacao"] * 0.80
    ...
    # Penaliza excessivamente configuração com poucos trades demais.
    if r["trades_total_validacao"] > 0 and r["trades_total_validacao"] < 250:
        score -= 1500.0
...
def main():
    print("=====================================================")
    print("V7 - V5.1 MOTOR + V5.5 FILTRO + GESTÃO DIÁRIA")
    print("=====================================================")
    ...
    print("\n=====================================================")
    print("RESUMO CONFIGS V7 - TOP 40")
    print("=====================================================")
    ...
    print("\n=====================================================")
    print("MELHOR CONFIG V7 ESCOLHIDA NAS ROTAÇÕES")
    print("=====================================================")
    ...
    aceitos_2026 = pred_2026_final[pred_2026_final["aceito_v7"]].copy()
    resultado_2026 = resumo(aceitos_2026, "TESTE_FINAL_2026_V7")
    ...
    mensal_2026 = mensal(aceitos_2026, "TESTE_FINAL_2026_V7")
    ...
    print("\n=====================================================")
    print("RESULTADO TESTE FINAL 2026 - V7")
    print("=====================================================")
    print(pd.Series(resultado_2026).to_string())

    print("\n=====================================================")
    print("MENSAL TESTE FINAL 2026 - V7")
    print("=====================================================")
    print(mensal_2026.to_string(index=False))

    print("\n=====================================================")
    print("DISTRIBUIÇÃO DIÁRIA V7 - 2026")
    print("=====================================================")
    ...
    config = {
        "nome": "V7_V51_MOTOR_V55_FILTRO_GESTAO_DIA",
        "descricao": "V5.1 como motor principal, V5.5 como filtro de risco e gestão diária.",
