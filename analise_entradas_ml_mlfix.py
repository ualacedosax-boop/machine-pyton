...
def carregar_ohlc(path: Path) -> pd.DataFrame:
    if path.suffix.lower() in [".xlsx", ".xls"]:
        ...
        # tenta leitura padrão e, se necessário, autodetecta separador
        try:
            df = pd.read_csv(path)
            if len(df.columns) == 1:
    ...
    # aceita "time" como datetime completo, como no seu arquivo:
    # 2025-06-01T19:00:00-03:00
    elif "time" in df.columns and "date" not in df.columns:
        df["datetime"] = pd.to_datetime(df["time"], errors="coerce")

    # aceita date + time separados
    ...
    # converte tudo para datetime sem fuso para comparar corretamente
...
def rodar_ml(amostra: pd.DataFrame, pasta_saida: Path):
    ...
    df = amostra.copy()
    y = df["side"].map({"BUY": 1, "SELL": 0}).astype(int)
    ...
    if not feature_cols:
        raise ValueError("Nenhuma feature numérica disponível para ML.")

    X = df[feature_cols].copy()

    # remove colunas totalmente vazias ou constantes
    valid_cols = []
    for c in X.columns:
        serie = pd.to_numeric(X[c], errors="coerce")
    ...
    if len(valid_cols) < 3:
        raise ValueError("Poucas features úteis para ML após limpeza.")
    ...
    # Se alguma classe tiver poucos exemplos, reduzimos o teste para garantir representatividade
    ...
    # remove novamente colunas constantes após imputação no treino
    cols_final = [c for c in X_train_imp.columns if X_train_imp[c].nunique(dropna=True) > 1]
    if len(cols_final) < 3:
        raise ValueError("Poucas features úteis para ML após imputação.")

    X_train_imp = X_train_imp[cols_final]
    X_test_imp = X_test_imp[cols_final]
    ...
    predicoes = pd.DataFrame({
        "datetime": df.loc[X_test_imp.index, "datetime"].astype(str).values,
        "side_real": df.loc[X_test_imp.index, "side"].values,
    ...
    metricas = {
        ...
        "n_features": int(len(cols_final)),
        "features_usadas": cols_final,
    ...
    predicoes.to_csv(pasta_saida / "05b_predicoes_ml.csv", index=False)
...
def criar_score_operacional(amostra: pd.DataFrame, ranking: pd.DataFrame, n_top: int = 6):
    """
    Cria um score simples BUY/SELL baseado nas melhores variáveis do ranking estatístico.
    Score > 0 favorece BUY; score < 0 favorece SELL.
    """
    df = amostra.copy()
    top = ranking.head(n_top).copy()
    ...
    for _, r in top.iterrows():
        ...
        serie = pd.to_numeric(df[feat], errors="coerce")
        med_buy = float(r["median_buy"])
        med_sell = float(r["median_sell"])
        efeito = float(r["effect_size_std"])

        std = float(serie.std(skipna=True))
        if not np.isfinite(std) or std == 0:
            std = 1.0

        z = (serie - serie.median(skipna=True)) / std

        # Se efeito < 0, valores menores favorecem BUY; se > 0, maiores favorecem BUY
        sinal = -1.0 if efeito < 0 else 1.0
        comp = sinal * z.fillna(0.0)

        score += comp
        componentes.append({
            "feature": feat,
            "effect_size_std": efeito,
            "median_buy": med_buy,
            "median_sell": med_sell
        })

    df["score_operacional"] = score
    df["score_sinal"] = np.where(df["score_operacional"] >= 0, "BUY", "SELL")
    df["score_acertou"] = (df["score_sinal"] == df["side"]).astype(int)
...
def salvar_resumo_txt(pasta_saida: Path, metricas: dict | None, ranking: pd.DataFrame, amostra: pd.DataFrame):
    ...
    if metricas:
        ...
        if metricas.get("roc_auc") is not None:
            linhas.append(f"- roc_auc: {metricas['roc_auc']:.4f}")
...
def main():
    ...
    amostra_score, score_componentes = criar_score_operacional(amostra, ranking, n_top=6)
    amostra_score.to_csv(pasta_saida / "01b_entradas_com_score.csv", index=False)
    score_componentes.to_csv(pasta_saida / "02b_componentes_score.csv", index=False)

    metricas = None
    try:
        metricas, _ = rodar_ml(amostra_score, pasta_saida)
    except Exception as e:
        with open(pasta_saida / "06_metricas_ml.json", "w", encoding="utf-8") as f:
            json.dump({"erro_ml": str(e)}, f, ensure_ascii=False, indent=2)

    salvar_resumo_txt(pasta_saida, metricas, ranking, amostra_score)
